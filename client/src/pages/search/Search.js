      ...
      <Container>
        <Form onSubmit={this.handleSubmit}>
          <Row className="justify-content-md-center mt-2 mb-4 ">
            <Col xs lg={6}>
              <Form.Group>
                <Form.Control ref={"searchTextRef"} as="input" type="text" placeholder="Search students by Name, Location, Learning Targets..."></Form.Control>
              </Form.Group>
            </Col>
            <Col md="auto">
              ...
              <Tab.Content>
                {this.state.list.map((user, index) => (
                  <Tab.Pane eventKey={`#user${index}`}>
                    ...
                    <Button className="mt-3 start" onClick={(e) => { e.preventDefault(); this.onstartChat(user)}}>Start Chat</Button>
                  </Tab.Pane>
                ))}
              </Tab.Content>
            </Col>
